...
am_bsp_iom_i2c_pins_enable(uint32_t ui32Module)
{
    switch(ui32Module)
    {
#ifdef AM_BSP_GPIO_IOM0_I2C_SCK
        case 0:
            am_bsp_pin_enable(IOM0_I2C_SCK);
            am_bsp_pin_enable(IOM0_I2C_SDA);
            break;
#endif

#ifdef AM_BSP_GPIO_IOM1_I2C_SCK
        case 1:
            am_bsp_pin_enable(IOM1_I2C_SCK);
            am_bsp_pin_enable(IOM1_I2C_SDA);
            break;
#endif
        //
        // If we get here, the caller's selected IOM interface couldn't be
...
#ifdef AM_BSP_GPIO_IOM0_I2C_SCK
        case 0:
            am_bsp_pin_disable(IOM0_I2C_SCK);
            am_bsp_pin_disable(IOM0_I2C_SDA);
            break;
#endif

#ifdef AM_BSP_GPIO_IOM1_I2C_SCK
        case 1:
            am_bsp_pin_disable(IOM1_I2C_SCK);
            am_bsp_pin_disable(IOM1_I2C_SDA);
            break;
#endif
        //
        // If we get here, the caller's selected IOM interface couldn't be
