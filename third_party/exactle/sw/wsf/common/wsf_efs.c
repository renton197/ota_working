  ...
  if (wsfEfsMediaValid(media))
  {
    ...
    for (i=0; i<WSF_EFS_MAX_FILES; i++)
    {
      if (wsfEfsFileOverlap(i, media, address, size))
      {
        ...
        /* Note: This assert can be removed after unit testing */
        WSF_ASSERT(address >= assertTmp);
        assertTmp = address;
      }
    }
...
void WsfEfsInit(void)
{
  ...
  memset(wsfEfsMediaTbl, 0, sizeof(wsfEfsMediaTbl));
 ...
 *
 *  \param  handle    Handle identifing the file.
 *
 ...
 *
 *  \param  handle    Handle identifing the file.
 *  \param  pAttr     Pointer to memory to strore the attributes.
 *
 *  \return WSF_EFS_SUCCESS or WSF_EFS_FAILURE.
 */
/*************************************************************************************************/
 ...
 */
/*************************************************************************************************/
uint8_t *WsfEfsGetFileName(wsfEfsHandle_t handle)
{
  wsfEfsControl_t *pFile;
 ...
 */
/*************************************************************************************************/
uint8_t *WsfEfsGetFileVersion(wsfEfsHandle_t handle)
{
  wsfEfsControl_t *pFile;
